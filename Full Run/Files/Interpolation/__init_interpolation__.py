...
#         return dwellings_cols
    ...
    """
    Purpose: Get sequences based on enumerator's daily sequence
    returns: dataframe with ward column, dwelling column, CENSUS_ENUMERATOR_NUM, CENSUS_ENUMERATOR_DATE
    """
    def applying_sequencing_only_enum(self, target_column = "CD_BLOCK_NUM"):
        sequences_dfs = []
        sequences_dfs.append(self.get_dwellings_dwellings_seq())
        dwellings_dist = self.get_enum_seq(all = True)
        dwellings_dist.dropna(subset=[target_column], inplace=True)
        dwellings_dist = dwellings_dist.groupby([self.ward_col, "enum_seq"], as_index=False).apply(
            lambda x: sequences.col_for_seq(x, X=self.x_col, Y=self.y_col))
        dwellings_dist = dwellings_dist.groupby([self.ward_col, "enum_seq"], as_index=False).apply(
            lambda x: sequences.get_dist_seq(x, 0))
        dwellings_dist.rename(columns = {"dist":"enum_dist", "sequence_len":"enum_sequence_len", "sequence_id":"enum_dist_id", "sequence_order_enum":"enum_dist_order"}, inplace = True)
        sequences_dfs.append(dwellings_dist.loc[:, [self.ward_col, self.dwelling_col, "enum_dist_id","enum_dist", "enum_dist_order","enum_sequence_len", "enum_seq", "num_between", "CENSUS_ENUMERATOR_NUM", "CENSUS_ENUMERATOR_DATE"]].copy())
        sequences_df1 = sequences_dfs[0]
        sequences_df2 = sequences_dfs[1]
        sequences_dfs = sequences_df2.merge(sequences_df1, on = [self.ward_col, self.dwelling_col], how = "left")
        self.df = self.data.merge(sequences_dfs, on = [self.ward_col, self.dwelling_col, "CENSUS_ENUMERATOR_NUM", "CENSUS_ENUMERATOR_DATE"], how = "left")
    ...
    def apply_sequencing(self, d = 0.1, n = 40, distance = False, fixed = False, dwelling = False, enumerator = False,tuned = False, enumerator_dist = False):
        ...
        if enumerator:
            ...
            sequences_dfs.append(dwellings_dist.loc[:, [self.ward_col, self.dwelling_col, "enum_dist_id", "enum_dist_order", "enum_dist", "enum_sequence_len", "enum_seq"]].copy())
