...
returns: dataframe
"""
def get_dist_seq(df, d, groupby_id = "sequence_id"):
#     print('d: ', d)
    df = df.copy()
    if d != 0:
        df["sequence_id"] = np.where(df["dist"] > d, df["dist"].index, np.nan)
    else: 
        df["sequence_id"] = df["dist"].index
...
#         df["sequence_id"] = df.tail(1).index[0]
    ...
    df["sequence_len"] = df.groupby(groupby_id)["num_between"].transform('sum')
    sequence_map = {ids:num for ids, num in zip(df[groupby_id].unique(), range(len(df[groupby_id].unique())))}
    df["sequence_order_enum"] = df.apply(lambda row: sequence_map[row[groupby_id]], axis = 1)
    return df
